...
// ---------------------------------------------------------------------------
// Resolve createIgnoreFilter
//
// Import dist/ignore-filter.js directly instead of the @understand-anything/core
// barrel (dist/index.js) — the barrel re-exports TreeSitterPlugin, which pulls
// web-tree-sitter/tree-sitter-*/zod/etc into node_modules at import time even
// though this script only needs createIgnoreFilter (ADR-5, design.md §6).
// pathToFileURL() is required on Windows because dynamic import() of raw
// "C:\..." paths throws ERR_UNSUPPORTED_ESM_URL_SCHEME.
// ---------------------------------------------------------------------------
const { createIgnoreFilter } = await import(
  pathToFileURL(resolve(pluginRoot, 'packages/core/dist/ignore-filter.js')).href
);
