 ...
 *
 * Input:  <project-root>/.open-tour/intermediate/scan-result.json
 * Output: <project-root>/.open-tour/intermediate/batches.json
 */
...
async function main() {
  ...
  const scanPath = join(projectRoot, '.open-tour', 'intermediate', 'scan-result.json');
  ...
  const exportsByPath = new Map(Object.entries(scan.exportsByPath || {}));
  ...
  const outPath = join(projectRoot, '.open-tour', 'intermediate', 'batches.json');
